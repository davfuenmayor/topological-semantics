...
(**The LFIs are a family of paraconsistent logics featuring a 'consistency' operator @{text "\<circ>"}
...
(**For LFIs we use the negation previously defined as @{text "\<C>\<^sup>d\<^sup>- = \<B>\<^sup>f\<^sup>p"}.*)
...
(**Observe that assumming CNTR \<B> we are allowed to exchange A and B variants.*)
