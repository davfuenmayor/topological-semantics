...
(**The LFUs are a family of paracomplete logics featuring a 'determinedness' operator \<currency>
...
(**For LFUs we use the negation previously defined as @{text "\<I>\<^sup>d\<^sup>- = \<C>\<^sup>-"}.*)
...
lemma "CLOSURE \<C> \<Longrightarrow> [\<turnstile> a \<^bold>\<or> \<^bold>\<not>a]" nitpick oops (**countermodel*)

(**We define two pairs of un/determinedness operators and show how they relate to each other.
...
(**Observe that assumming EXPN \<C> we are allowed to exchange A and B variants.*)
