...
(**In this section we examine some special paracomplete logics. The idea is to illustrate an approach by
...
(**Drawing upon the literature on Johanson's minimal logic, we introduce an unconstrained propositional
constant Q, which we then employ to define a 'rigid' frontier operation @{text "\<F>'"}.*)
consts Q::"\<sigma>"
...
(**As defined, @{text "\<F>'"} (and its corresponding closure operation) satisfies several semantic conditions.*)
...
(**The negation so defined validates some properties corresponding to a (rather weak) paracomplete logic:*)
...
(**Below we combine negation with strict implication and verify some interesting properties.
...
(**We now semantically characterize (an approximation of) Johansson's Minimal Logic along with some 
