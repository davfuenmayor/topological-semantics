...
section \<open>Subintuitionistic and subminimal logics\<close>
...
(**As defined, @{text "\<F>'"} satisfies already several semantic conditions. Moreover, adding Fr-3 as a
further restriction collapses into classical logic.*)
...
abbreviation neg_IC::"\<sigma>\<Rightarrow>\<sigma>" ("\<^bold>\<not>")  where  "\<^bold>\<not>\<phi> \<equiv> \<C>'(\<I>(\<^bold>\<midarrow>\<phi>))"
...
lemma negC_def: "\<forall>A. \<^bold>\<not>A \<^bold>\<approx> \<I>(\<^bold>\<midarrow>A) \<^bold>\<or> Q" by (simp add: Cl_fr_def)
lemma negI_def: "Fr_2 \<F> \<Longrightarrow> \<forall>A. \<^bold>\<not>A \<^bold>\<approx> \<C>(A) \<^bold>\<rightarrow> Q" using Cl_fr_def Fr_2_def Int_fr_def conn by auto

lemma "\<not>ECQ \<^bold>\<not> \<and> DM1 \<^bold>\<not> \<and> DM2 \<^bold>\<not> \<and> DM3 \<^bold>\<not> \<and> DM4 \<^bold>\<not>" nitpick[satisfy,card w=3] oops (**(weakly paraconsistent) model found*)
lemma "DM1 \<^bold>\<not> \<and> DM2 \<^bold>\<not> \<and> DM3 \<^bold>\<not> \<and> DM4 \<^bold>\<not> \<longrightarrow> ECQm \<^bold>\<not>" using Cl_fr_def ECQm_def IF2 dEXP_def unfolding conn by fastforce

lemma "DNI \<^bold>\<not> \<and> DNE \<^bold>\<not> \<longrightarrow> ECQ \<^bold>\<not>" using DNE_def ECQ_def Int_fr_def negC_def unfolding conn by (metis (no_types, lifting)) 

lemma "\<FF> \<F> \<Longrightarrow> TND \<^bold>\<not>" nitpick oops (*counterexample found*)
lemma "\<FF> \<F> \<Longrightarrow> TNDw \<^bold>\<not>" nitpick oops
lemma "\<FF> \<F> \<Longrightarrow> TNDm \<^bold>\<not>" nitpick oops
lemma "\<FF> \<F> \<Longrightarrow> ECQ \<^bold>\<not>" nitpick oops
lemma ECQw: "ECQw \<^bold>\<not>" using Cl_fr_def Disj_I ECQw_def unfolding conn by auto  
...
(**Below we combine negation with (strict-)implication to check some interesting properties.
...
(**The (weak, local) contraposition axiom is valid under appropriate conditions*)
...
(**Other (local) contraposition axioms are not valid:*)
lemma "\<FF> \<F> \<Longrightarrow> lCoP1(\<^bold>\<Rightarrow>) \<^bold>\<not>" nitpick oops
lemma "\<FF> \<F> \<Longrightarrow> lCoP2(\<^bold>\<Rightarrow>) \<^bold>\<not>" nitpick oops
lemma "\<FF> \<F> \<Longrightarrow> lCoP3(\<^bold>\<Rightarrow>) \<^bold>\<not>" nitpick oops
(**And this time disjunctive syllogism cannot be validated:*)
lemma "\<FF> \<F> \<Longrightarrow> DS1(\<^bold>\<Rightarrow>) \<^bold>\<not>" nitpick oops
lemma "\<FF> \<F> \<Longrightarrow> DS2(\<^bold>\<Rightarrow>) \<^bold>\<not>" nitpick oops
...
(**Check different variants of modus tollens.*)
...
(**We now define (an approximation of) Johansson's Minimal Logic together with some exemplary
'subminimal' logics (note that many more are possible). We then check some relevant properties.*)
...
(**TND*)
lemma "JML \<Longrightarrow> TND \<^bold>\<not>" nitpick oops
...
lemma "SML1 \<Longrightarrow> DNI \<^bold>\<not>" nitpick oops
(**DNE*)
lemma "JML \<Longrightarrow> DNE \<^bold>\<not>"  nitpick oops
(**CoPw*)
...
lemma "SML1 \<Longrightarrow> CoP1 \<^bold>\<not>" nitpick oops
(**CoP2/3*)
lemma "JML \<Longrightarrow> CoP2 \<^bold>\<not>" nitpick oops
lemma "JML \<Longrightarrow> CoP3 \<^bold>\<not>" nitpick oops
(**XCoP*)
